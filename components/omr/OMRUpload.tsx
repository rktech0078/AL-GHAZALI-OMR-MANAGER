...
export default function OMRUpload({ examId, studentId, onSuccess, onError }: OMRUploadProps) {
    ...
    // Define stopCamera before the useEffect that uses it
            ...
            {cameraActive && (
                <div className="fixed inset-0 z-50 bg-black flex flex-col">
                    ...
                    <div className="bg-black/90 p-6 pb-10 flex items-center justify-between">
                        <button
                            onClick={stopCamera}
                            className="text-white p-4 rounded-full hover:bg-white/10 transition"
                        >
            ...
            <div className="space-y-8">
                ...
                {!selectedFile ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {/* Drag & Drop Zone */}
                        <div
                            className={`relative border-2 border-dashed rounded-3xl p-10 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[300px] cursor-pointer group
                        ...
                        <div
                            onClick={startCamera}
                            className="relative border-2 border-gray-200 rounded-3xl p-10 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[300px] cursor-pointer hover:border-indigo-400 hover:bg-gray-50 group"
                        >
                                    ...
                                    <>
                                        ...
                                        {uploading ? (
                                            <div className="space-y-4">
                                                ...
                                                <p className="text-center text-sm font-medium text-indigo-600 animate-pulse">
                                                    Analyzing bubbles & QR code...
                                                </p>
                                            </div>
                                        ) : (
                                    ...
                                    <div className="text-center animate-slide-up">
                                        ...
                                        <div className="bg-gray-50 rounded-2xl p-4 space-y-3">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <span className="text-xl font-bold text-gray-900">{result.obtainedMarks}/{result.totalMarks}</span>
                                            </div>
                                            <div className="flex justify-between items-center">
                                                <span className="text-gray-600">Percentage</span>
                                                <span className="text-lg font-bold text-indigo-600">{result.percentage}%</span>
                                            </div>
                                            <div className="flex justify-between items-center">
                                                <span className="text-gray-600">Grade</span>
                                                <span className={`text-lg font-bold ${result.status === 'pass' ? 'text-green-600' : 'text-red-600'}`}>
                                                    {result.grade}
                                                </span>
                                            </div>
                                            <div className="pt-2 mt-2 border-t border-gray-200">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span className="text-xs font-medium text-gray-700 bg-white px-2 py-1 rounded-full">
                                                        {result.processingMethod === 'library' ? '✓ Standard Scan' :
                                                            result.processingMethod === 'hybrid' ? '⚡ Enhanced (AI)' :
                                                                result.processingMethod === 'groq-ai' ? '🤖 AI (Groq)' :
                                                                    result.processingMethod === 'gemini-ai' ? '🛡️ AI (Gemini)' : 'AI'}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-center mt-2">
                                                    <span className="text-xs text-gray-500">Accuracy</span>
                                                    <span className="text-xs font-medium text-green-600">{Math.round(result.confidence * 100)}%</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <button
                                            onClick={handleReset}
                                            className="mt-6 w-full py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-medium transition-colors"
                                        >
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-scale-in">
                        <div className="p-6 border-b border-gray-100">
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                We couldn't read the QR code. Who does this paper belong to?
                            </p>
                        </div>
                        <div className="p-4">
                            <input
                                ...
                                className="w-full px-4 py-3 border border-gray-200 rounded-xl mb-4 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                                autoFocus
                            />
                                ...
                                {loadingStudents ? (
                                    ...
                                    filteredStudents.map(student => (
                                        <div
                                            ...
                                            className={`p-3 rounded-xl cursor-pointer border transition-all ${selectedStudentId === student.id
